"""WebSocket bridge between Python and the pmr UXP plugin inside Premiere.
...
network connections. So pmr inverts the connection: this module hosts a
WebSocket server on localhost, and the bundled `pmr bridge` panel inside
...
logger = logging.getLogger("pmr.bridge")
...
class Bridge:
    ...
    def start(self) -> None:
        ...
        if self._thread is not None:
            return
        self._thread = threading.Thread(target=self._run_loop, name="pmr-bridge", daemon=True)
        self._thread.start()
        ...
        if self._start_error is not None:
            ...
            if isinstance(err, OSError):
                raise errors.ConnectionError(
                    ...
                    cause="Another pmr process (daemon, MCP server, or CLI) already owns the port.",
                    fix="Use the running `pmr serve` daemon, or stop the other process.",
    ...
    def request(self, op: str, *, timeout: float = 60.0, **payload: Any) -> Any:
        ...
        if loop is None:
            raise errors.BridgeError(
                "Bridge is not running.", fix="Call Bridge.start() or use pmr.Premiere()."
            )
        with self._conn_lock:
            conn = self._conn
        if conn is None:
