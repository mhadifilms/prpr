"""``pmr completion`` — shell completion script generation.
...
_INSTALL_HINTS = {
    "bash": (
        'Add to ~/.bashrc:\n  eval "$(_PMR_COMPLETE=bash_source pmr)"\n'
        "Or save the output to ~/.local/share/bash-completion/completions/pmr"
    ),
    "zsh": (
        'Add to ~/.zshrc:\n  eval "$(_PMR_COMPLETE=zsh_source pmr)"\n'
        "Or save the output to a file in $fpath named _pmr."
    ),
    "fish": ("Save the output to ~/.config/fish/completions/pmr.fish"),
}
...
def show(
    ...
    rc = shell_complete(click_command, {}, "pmr", "_PMR_COMPLETE", f"{shell}_source")
    if rc != 0:
