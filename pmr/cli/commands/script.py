"""``pmr eval`` / ``pmr exec`` / ``pmr repl`` — scripting escape hatches.

Unlike dvr (whose ``eval`` runs *Python* against the Resolve API), pmr's
``eval`` and ``exec`` run **JavaScript inside Premiere** via the bridge
plugin (``ppro`` and ``uxp`` are in scope). ``repl`` stays a Python REPL
with a live :class:`~pmr.premiere.Premiere` handle bound to ``p``.
"""
...
def register(app: typer.Typer) -> None:
    ...
    def eval_cmd(
        ...
        Examples:
            ...
            pmr eval "ppro.Project.getActiveProject()"
            pmr eval "(await ppro.Project.getActiveProject()).name"
            pmr eval "return (await ppro.Project.getActiveProject()).name"
        """
        cfg = ctx.obj or {}
    ...
    def repl_cmd(ctx: typer.Context) -> None:
        ...
        ns = _ns(p)
        banner = (
            f"pmr repl — {p.app.product} {p.app.version}\n"
            "Available: p, project, timeline, pmr\n"
