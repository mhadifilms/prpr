"""``pmr serve`` sub-commands — daemon mode."""
...
def start(
    ...
    # the root command, so they must come after "pmr" but before "serve".
    cmd = [sys.executable, "-m", "pmr.cli.main"]
    if not auto_launch:
    ...
    proc = subprocess.Popen(
        ...
        env={**os.environ, "PMR_FORMAT": cfg.get("format") or "json"},
    )
    output.emit(
...
def status_cmd(ctx: typer.Context) -> None:
    """Show daemon status."""
    cfg = ctx.obj or {}
    output.emit(daemon.status(), fmt=cfg.get("format"), headline="pmr serve")
