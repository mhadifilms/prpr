"""``pmr spec`` sub-commands — spec file tooling."""
...
def export(
    ...
    """Build a spec from the live project state (the inverse of `pmr apply`).
    ...
    spec file and managed with `pmr plan` / `pmr apply` from then on.
    """
    p = _premiere(ctx)
