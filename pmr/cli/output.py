...
* ``PMR_FORMAT`` env var               persistent default
...
no ANSI codes), so ``pmr ... | jq`` Just Works. Tables use ``rich`` and
...
# outside the command (e.g. the top-level PmrError catch in main) render
