...
Errors come back as :class:`pmr.errors.PmrError.to_dict` payloads
...
are still registered and raise :class:`pmr.errors.NotSupportedError`,
...
instead of an unknown-tool failure. See ``pmr.schema.PARITY``.
...
try:
    ...
    raise ImportError(
        "The MCP server requires the `mcp` package. Reinstall with `pip install pmr`."
    ) from exc

logger = logging.getLogger("pmr.mcp")
...
class _PremiereCache:
    ...
    def __init__(
        ...
        *,
        ...
        self._error: errors.PmrError | None = None
...
def _h_doctor(ctx: _Context, args: dict[str, Any]) -> dict[str, Any]:
    """Diagnose the pmr <-> Premiere setup without raising on failure.
...
# ---------------------------------------------------------------------------
# Handlers — effects / transitions / components (pmr-only)
# ---------------------------------------------------------------------------
...
def _h_interchange_import(_ctx: _Context, args: dict[str, Any]) -> dict[str, Any]:
    ...
    # truth for the message lives in pmr.interchange.
...
def _h_eval(ctx: _Context, args: dict[str, Any]) -> Any:
    if os.environ.get("PMR_MCP_ENABLE_EVAL", "0") not in ("1", "true", "yes"):
        raise errors.PmrError(
            ...
            cause=(
                ...
                "contexts; PMR_MCP_ENABLE_EVAL is not set."
            ),
            fix=(
                "Restart the MCP server with PMR_MCP_ENABLE_EVAL=1 in its environment "
...
def build_registry() -> list[_ToolSpec]:
    return [
        ...
        _ToolSpec(
            name="version",
            description="Return the pmr package version, Python version, and platform.",
            handler=_h_version,
        ...
        _ToolSpec(
            name="doctor",
            description=(
                "Diagnose the pmr -> Adobe Premiere Pro setup. Reports installed apps, "
        ...
        _ToolSpec(
            ...
            description=(
                ...
                "dvr<->pmr support matrix), marker-types, marker-colors, export-types, "
        ...
        # ---- effects / transitions (pmr-only) ------------------------------
        _ToolSpec(
            name="effects_list",
            description=(
                "List available effects by kind: video (match names + display names), "
                "audio (display names), or transition (match names). pmr-only tool."
            ),
            schema=_schema(
                {
                    "kind": {
        ...
        _ToolSpec(
            ...
            description=(
                ...
                "(video: matchName from effects_list; audio: display name). pmr-only tool."
            ),
            schema=_schema(
                {
                    "name": {"type": "string"},
        ...
        _ToolSpec(
            ...
            description=(
                ...
                "Get match names from effects_list kind=transition. pmr-only tool."
            ),
            schema=_schema(
                {
                    "match_name": {
        ...
        _ToolSpec(
            ...
            description=(
                ...
                "values when with_values=true. pmr-only tool."
            ),
            schema=_schema(
                {
                    "clip_name": {"type": "string"},
        ...
        _ToolSpec(
            ...
            description=(
                ...
                "PMR_PRESET_DIRS). Does not require Premiere."
            ),
        ...
        _ToolSpec(
            ...
            description=(
                ...
                "name or media path. kind: xmp | project (default: both). pmr-only tool."
            ),
            schema=_schema(
                {
                    "name": {"type": "string"},
        ...
        _ToolSpec(
            name="plugin_install",
            description=(
                "Install (or upgrade) the pmr bridge UXP plugin via Adobe's plugin "
                "installer (UPIA). After installing, open Window > UXP Plugins > "
                "pmr bridge in Premiere."
            ),
        ...
        _ToolSpec(
            ...
            description=(
                ...
                "PMR_MCP_ENABLE_EVAL=1 is set in the server's environment."
            ),
...
def _build_resource_registry() -> list[_ResourceSpec]:
    ...
    specs = [
        ...
        _ResourceSpec(
            ...
            description="Static pmr <-> Premiere environment diagnosis (no connection attempt).",
...
def _err(exc: errors.PmrError | Exception) -> CallToolResult:
    if isinstance(exc, errors.PmrError):
...
def _dispatch(
    ...
    if spec is None:
        return _err(errors.PmrError(f"Unknown tool: {name!r}"))
    ...
    try:
        ...
        return _err(errors.PmrError(f"{type(exc).__name__}: {exc}"))
    return _ok(value)


def tools_summary(detail: bool = False) -> list[dict[str, Any]]:
    """Return tools as plain dicts for the `pmr mcp tools` CLI command.
...
def build_server(*, auto_launch: bool = True, timeout: float = 30.0) -> Server:
    """Construct an MCP Server with all `pmr` tools and resources registered."""
    server = Server("pmr")
