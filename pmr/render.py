...
queue** like Resolve's. ``pmr`` keeps dvr's ``render`` namespace:

- ``presets()``   discovers ``.epr`` files (user + AME + ``PMR_PRESET_DIRS``)
...
def preset_search_dirs() -> list[Path]:
    ...
    dirs: list[Path] = []
    env = os.environ.get("PMR_PRESET_DIRS")
    if env:
...
def resolve_preset(name_or_path: str | None) -> str | None:
    ...
    raise errors.RenderError(
        ...
        fix="Pass a path to a .epr file, or run `pmr render presets` to list "
...
class RenderNamespace:
    ...
    def queue(self) -> list[dict[str, Any]]:
        raise errors.NotSupportedError(
            ...
            fix="Use `pmr render submit --wait` and `pmr render watch` for status; "
    ...
    def formats(self) -> list[dict[str, Any]]:
        raise errors.NotSupportedError(
            ...
            fix="Use `pmr render presets` to discover .epr presets instead.",
        )
