...
PARITY: dict[str, dict[str, Any]] = {
    ...
    "clip.set_properties": {"status": "dvr-only", "reason": "no generic clip-property dict; use effects.set_param"},
    "clip.transform": {"status": "both", "note": "pmr: effects.set_param on the Motion/Opacity components"},
    ...
    "project.color_settings": {"status": "both", "note": "pmr: graphics white luminance only"},
    "project.import_sequences": {"status": "pmr-only", "reason": "cross-project sequence import"},
    "project.import_ae_comps": {"status": "pmr-only", "reason": "After Effects comp import"},
    "media.footage_interpretation": {"status": "pmr-only", "reason": "FootageInterpretation API"},
    "media.purge_cache": {"status": "pmr-only", "reason": "MediaManager.purgeMediaCache (26.5+)"},
    "timeline.track_update": {"status": "both", "note": "pmr: mute + rename (26.3+); no lock/add/remove"},
