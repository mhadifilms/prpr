"""Point-in-time project snapshots — `pmr snapshot`.
...
def snapshots_dir() -> Path:
    target = Path.home() / ".pmr" / "snapshots"
...
def load(name: str) -> Snapshot:
    ...
    if not path.exists():
        raise errors.PmrError(
            ...
            fix="Run `pmr snapshot list` to see saved snapshots.",
...
def delete(name: str) -> None:
    path = snapshots_dir() / f"{_safe_name(name)}.json"
    if not path.exists():
        raise errors.PmrError(f"Snapshot not found: {name}")
    path.unlink()
