"""``prpr clip`` — query and mutate clips on the active sequence.

Premiere's UXP API has no generic clip-property dict (dvr's ``--where``
+ ``set`` model), so filtering here uses simple flags::

    prpr clip ls --track video --duration-gt 2.5
    prpr clip ls --name-contains interview --track-index 1
    prpr clip disable "b-roll.mp4" --track video
"""
