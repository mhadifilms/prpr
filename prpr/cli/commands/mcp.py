"""``prpr mcp`` sub-commands -- MCP server for LLM agents.

The server implementation lives in :mod:`prpr.mcp.server` (functions
...
app = typer.Typer(name="mcp", help="MCP server: expose prpr to LLM agents.")


def _import_mcp_server() -> Any:
    """Import prpr.mcp.server lazily; exit with a clear message if missing."""
    try:
        from ... import mcp as mcp_pkg  # noqa: F401 — ensure the package resolves
        from ...mcp import server as mcp_server
    except ImportError as exc:
        typer.echo(
            "MCP support is unavailable (prpr.mcp.server not found or its "
            "dependency is missing). Reinstall with `pip install prpr`.",
            err=True,
        )
...
def tools(
    ...
    """List the MCP tools prpr exposes to LLM agents."""
    mcp_server = _import_mcp_server()
    ...
    output.emit(summary, fmt=fmt, headline=f"{len(payload)} prpr MCP tools")
...
def _pmr_command() -> list[str]:
    """Return [command, ...args] for spawning the prpr MCP server."""
    pmr_path = shutil.which("prpr")
    if pmr_path:
        return [pmr_path, "mcp", "serve"]
    # Fallback: invoke through the active Python interpreter.
    return [sys.executable, "-m", "prpr.cli.main", "mcp", "serve"]
...
def _build_server_entry(
    *,
    ...
    if enable_eval:
        server_entry["env"] = {"PRPR_MCP_ENABLE_EVAL": "1"}
...
_EnableEvalOpt = Annotated[
    ...
    typer.Option(
        "--enable-eval",
        help="Enable the `eval` tool by setting PRPR_MCP_ENABLE_EVAL=1 in the server's env.",
    ),
]
...
def install(
    ...
    """Generic installer: write the prpr MCP server entry into any client config
...
def install_claude(
    ...
    """Add (or update) the prpr MCP server in Claude Desktop's config.
...
def install_cursor(
    ...
    """Add (or update) the prpr MCP server in Cursor's config (~/.cursor/mcp.json)."""
...
def uninstall(
    ...
    """Remove the prpr MCP server entry from client config(s)."""
...
def install_claude_code(
    ...
    enable_eval: Annotated[
        ...
        typer.Option(
            "--enable-eval",
            help="Enable the `eval` tool (sets PRPR_MCP_ENABLE_EVAL=1).",
        ),
    ] = False,
    no_launch: Annotated[
        bool,
    ...
    """Add (or update) the prpr MCP server in Claude Code's config (via `claude` CLI).

    Equivalent to running::

        claude mcp add -s user prpr -- prpr --no-launch --timeout 5 mcp serve

    plus optional ``-e PRPR_MCP_ENABLE_EVAL=1`` and ``--force`` handling.
    ...
    if not claude_path:
        typer.echo(
            ...
            "or use `prpr mcp install-claude` for Claude Desktop instead.",
            err=True,
        )
        raise typer.Exit(1)
    ...
    if enable_eval:
        add_cmd.extend(["-e", "PRPR_MCP_ENABLE_EVAL=1"])
    add_cmd.append(name)
    add_cmd.append("--")
