...
This module wires the public library to the ``prpr`` shell command. It is
...
under ``prpr/cli/commands/`` and register it here.
"""
...
def root(
    ...
    version: Annotated[
        ...
        typer.Option(
            ...
            is_eager=True,
            help="Print the prpr version and exit.",
        ),
    ] = None,
) -> None:
...
def inspect(ctx: typer.Context) -> None:
    ...
    with _premiere_session(ctx) as p:
        output.emit(p.inspect(), fmt=ctx.obj["format"], headline="prpr inspect")
...
def doctor_cmd(
    ...
    """Diagnose the prpr <-> Premiere setup: apps, plugin, ports, connectivity."""
    ...
    output.emit(report, fmt=cfg.get("format"), headline="prpr doctor")
...
def _should_bypass_daemon(argv: list[str]) -> bool:
    ...
    if sys.platform == "win32":
        return True
    if os.environ.get("PRPR_NO_DAEMON", "").strip().lower() in ("1", "true", "yes"):
        return True
...
def _forward_to_daemon(argv: list[str]) -> int | None:
    ...
    if not daemon.socket_path().exists():
        if os.environ.get("PRPR_DAEMON", "").strip().lower() not in ("1", "true", "auto", "yes"):
            return None
...
def _spawn_daemon() -> bool:
    """Start a background daemon (PRPR_DAEMON=auto); wait briefly for the socket."""
    import subprocess
    import time
    ...
    subprocess.Popen(
        [sys.executable, "-m", "prpr.cli.main", "serve", "start", "--foreground"],
...
def main() -> None:
    ...
    Transparently forwards to a running prpr daemon (persistent bridge
    connection, no per-command plugin handshake) when one is available,
    and catches :class:`~prpr.errors.PrprError` from *any* command so
