...
Every ``prpr`` sub-command shares this one connection factory, and
uncaught :class:`~prpr.errors.PrprError` exceptions are rendered as
structured output by the top-level handler in :mod:`prpr.cli.main` — so
commands can simply let library errors propagate.

When the CLI runs *inside* the prpr daemon (see :mod:`prpr.daemon`), the
