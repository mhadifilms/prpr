...
that endpoint (the :class:`prpr.bridge.Bridge`) and serves CLI/library
clients over a Unix-domain socket, so many short-lived `prpr` commands
...
allow-list, plus ``cli`` which runs any prpr CLI command in-process.

Socket location: ``$XDG_RUNTIME_DIR/prpr/prpr.sock`` if set, otherwise
...
logger = logging.getLogger("prpr.daemon")
...
def run_cli(argv: list[str]) -> dict[str, Any]:
    """Execute a prpr CLI command in this process; return stdout/stderr/exit code."""
    import io
    ...
    with _CLI_LOCK, redirect_stdout(stdout), redirect_stderr(stderr):
        try:
            rv = app(args=list(argv), prog_name="prpr", standalone_mode=False)
...
def _dispatch(premiere: Premiere, method: str, params: Any) -> Any:
    if method not in _METHODS:
        raise errors.PrprError(
            ...
            fix="See `prpr serve methods` for the allow-list.",
        )
...
def serve(*, auto_launch: bool = True, timeout: float = 60.0) -> None:
    ...
    if sock.exists():
        if _ping_existing():
            raise errors.PrprError(
                f"A prpr daemon is already running at {sock}.",
                fix="Run `prpr serve stop` first.",
            )
        sock.unlink()
    ...
    def call(self, method: str, params: Any = None) -> Any:
        if not self._path.exists():
            raise errors.ConnectionError(
                "No prpr daemon is running.",
                fix="Run `prpr serve start` first, or omit the daemon and use direct mode.",
        ...
        if not data.strip():
            raise errors.ConnectionError(
                "The prpr daemon closed the connection without replying.",
                cause="The daemon likely crashed or was stopped mid-request.",
                fix="Check `prpr serve status`; restart with `prpr serve start`.",
                state={"method": method},
            )
