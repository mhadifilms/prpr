...
def diagnose(
    ...
    """Return a diagnostic dict describing this machine's prpr setup."""
    ...
    if not plugin["installed"]:
        problems.append("The prpr bridge plugin is not installed.")
        fixes.append(
            "Run `prpr plugin install`, then restart Premiere (the headless bridge auto-starts)."
        )
    if not result["premiere_running"]:
        problems.append("Premiere Pro is not running.")
        fixes.append("Launch Premiere (prpr auto-launches it unless --no-launch).")
    ...
    if probe:
        try:
            ...
            if fresh["stale"]:
                problems.append(
                    f"Bridge plugin is v{fresh['connected']} but prpr ships v{fresh['bundled']}."
                )
                fixes.append("Run `prpr plugin install` and restart Premiere to update the bridge.")
